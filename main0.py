...
def main(event, context):
    ...
    utils.clearLog()
    savePoint(
        'https://eu-proxypool.herokuapp.com/clash/proxies?nc=CN&speed=10', 'eu-proxypool')

    savePoint(
        'https://free886.herokuapp.com/clash/proxies?nc=CN', 'free886')
      
    savePoint(
        'http://guobang.herokuapp.com/clash/proxies?nc=CN&speed=10', 'guobang')

    savePoint(
        'https://hellopool.herokuapp.com/clash/proxies?nc=CN&speed=50', 'hellopool')
    
    savePoint(
        'https://free.jingfu.cf/clash/proxies?nc=CN&speed=40', 'jingfu')
...
#        'https://fq.lonxin.net/clash/proxies?nc=CN&type=trojan', 'lonxin')
    ...
    savePoint(
        'https://sspool.herokuapp.com/clash/proxies?nc=CN&speed=20', 'sspool')
    
    savePoint(
        'https://us-proxypool.herokuapp.com/clash/proxies?nc=CN&speed=10', 'us-proxypool')
